...
const ywsUtils = async function() {
  return import('./utils.mjs')
}
...
wss.on('connection', async (conn, req) => {
  (await ywsUtils()).setupWSConnection(conn, req, { gc: false, docName: "text" })
})

// log some stats
setInterval(async () => {
  let conns = 0
  const utils = await ywsUtils();
  utils.docs.forEach(doc => { conns += doc.conns.size })
  const stats = {
    conns,
    docs: utils.docs.size,
